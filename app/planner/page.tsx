...
export default function PlannerPage() {
  ...
  // Auto-generate the default 30-day projection on first load.
  // Wait one extra render if the buffer needs to be seeded from settings so the
  // projection uses the correct cash buffer instead of 0.
  useEffect(() => {
    if (hasAutoGenerated.current) return;
    if (cashSources === undefined || settings === undefined) return;
    const settingsBuffer = settings.minimumCashBufferMinor ?? 0;
    if (settingsBuffer > 0 && bufferMinor === 0) return;
    hasAutoGenerated.current = true;
    generate();
  }, [cashSources, settings, bufferMinor, generate]);
    ...
    <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
      ...
      {!result && running && (
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card p-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">Generating your plan…</p>
        </div>
      )}

      {!result && !running && (
        <div className="rounded-lg border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
          Adjust the period or strategy above, then click <strong>Generate Plan</strong>.
        </div>
      )}
    </div>
  );
